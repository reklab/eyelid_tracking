function [ctrCoor,maxArea,expandedContour,angle,Xs,Ys,HSVRanges] = eye_edge_gs(frame,userYN,Xs,Ys,prevMask,gs)
...
if strcmp(gs,'RGB') == 1
    ...
    expandedContour = activecontour(frame,prevMask,15);
...
if strcmp(gs,'RGB') == 1
    ...
    hsvFr = (cat(3, zeros(size(frame)), zeros(size(frame)), double(frame)./255));
    hVals = hsvFr(ind1);
    sVals = hsvFr(ind2);
    vVals = hsvFr(ind3);
    meanH = mean(hVals);
